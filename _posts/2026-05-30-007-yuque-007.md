...
secondary_category: "算法与面试"
series: "算法面试"
primary_category_order: 1
secondary_category_order: 9
series_order: 1
post_order: 7
categories:
  - 人工智能技术
...
<font style="color:#3F3F3F;">以Llama 7B模型为例，hidden_size为4096，也就说每个K,V有4096 个数据，假设是半精度浮点数据float16，一个Transformer Block中就有 4096* 2(每个元素占2字节) *2(K和V) = 16KB的单序列 K,V缓存空间，而Llama 2一共32个Transformer Block，所以单序列整个模型需要16 * 32 = 512KB的缓存空间。</font>
...
int8(1字节)，int4(0.5字节)，float16(半<font style="color:#2C2C36;">精度浮点型</font>2字节)、float32(<font style="color:#2C2C36;">单精度浮点型，</font>4字节)、float64(<font style="color:#2C2C36;">双精度浮点型，</font>8字节)、bfloat16(2字节)
...
在计算注意力权重时，旋转Q和K确保了位置编码对权重计算的影响。然而，V并不直接参与权重计算，它仅在加权求和中作为信息传递的一部分，<font style="color:#2C2C36;">用来生成最终的注意力输出</font>。因此，将V保持原值可以避免不必要的干扰，确保信息的一致性和准确性。
