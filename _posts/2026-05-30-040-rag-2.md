...
3. **领域知识和长尾知识缺乏**: 对于特定垂直领域或私有知识库，通用 LLMs 可能缺乏足够的专业知识。相关研究指出 **LLMs 在学习长尾知识方面存在困难**([Kandpal et al., 2022](https://arxiv.org/abs/2211.08411))，而 RAG 正是解决这一问题的有效途径。
...
+ **文本分块 (Chunking / Segmentation)**: 将加载的文档分割成较小的文本块（chunks）。这是因为 LLMs 的上下文窗口长度有限（尽管现代 LLM 的上下文窗口越来越大，如 Gemini 1.5 Pro 的百万级 token 上下文，但过长的上下文仍可能导致**中间遗忘问题** ([Liu et al., 2023](https://arxiv.org/abs/2307.03172))，并且更小的文本块有助于提高检索的精确度。
...
+ **文本嵌入 (Text Embedding)**: 使用预训练的文本嵌入模型（如 OpenAI’s `text-embedding-3-small/large`）将每个文本块转换为高维向量。这些向量能够捕捉文本块的语义信息，使得语义相似的文本块在向量空间中距离更近。查询  也会被同一个嵌入模型转换为向量 ，而每个文本块  被转换为 。
...
**Fig. 3. Comparison of different ANN algorithms: Flat, LSH, HNSW, IVF. (Image source: **[**Pinecone**](https://www.pinecone.io/learn/vector-indexes/)**)**

图 3 展示了几种常见的向量索引类型及其在检索质量、速度和内存消耗方面的权衡。

### 2. 检索 (Retrieval)
当用户提出一个查询  (或 ) 时，RAG 系统会执行以下检索步骤：

1. **查询嵌入 (Query Embedding)**: 使用与文档处理阶段相同的嵌入模型将用户查询  转换为查询向量 。
2. **相似性搜索 (Similarity Search)**: 在向量数据库中，计算查询向量  与所有已索引文本块向量  之间的相似度。最常用的相似度度量是**余弦相似度 (Cosine Similarity)**：其中  表示向量点积， 表示向量的欧几里得范数（L2 范数）。其他相似度度量还包括：
    - **点积 (Dot Product / Inner Product)**: 。如果向量已归一化，则等价于余弦相似度。
    - **欧氏距离 (Euclidean Distance / L2 Distance)**: 。距离越小表示越相似。
3. **上下文选择 (Context Selection)**: 选择与查询向量最相似的  个文本块  作为增强 LLM 上下文的背景知识。 值的选择是一个超参数，需要根据任务和 LLM 的上下文窗口大小进行调整。

### 3. 生成 (Generation)
检索到相关的文本块后，RAG 系统将这些信息与原始用户查询结合起来，输入给 LLM 以生成最终答案：

1. **提示构建 (Prompt Engineering)**: 将用户查询  和检索到的  个文本块  组合成一个结构化的提示 。一个常见的提示模板如下：
...
2. **LLM 推理 (LLM Inference)**: 将构建好的提示  输入给 LLM，LLM 会基于其预训练知识和提供的上下文信息生成答案 。
...
**Fig. 4. The Evaluation of RAG system.**
...
+ **预检索策略 (Pre-retrieval Strategies)**: 这些策略在实际检索发生之前对数据或查询进行优化。
    ...
    - **索引结构优化 (Index Structure Optimization)**:
        * **分块策略优化 (Chunking Strategy Optimization)**:
            ...
            + **摘要嵌入 (Summary Embedding / Proposition Indexing)**: 为每个文档或大块文本生成摘要或原子性的命题（propositions），然后对摘要/命题进行嵌入和索引。检索时先匹配摘要/命题，再定位到原始文档或相关块。DenseX ([Chen et al., 2023d](https://syhya.github.io/zh/posts/2025-02-03-rag/#chen-et-al-2023d)) 提出使用命题作为检索单元。
    ...
    - **查询优化 (Query Optimization)**:
        ...
        * **HyDE (Hypothetical Document Embeddings)** ([Gao et al., 2022](https://arxiv.org/abs/2212.10496)): LLM 根据用户查询生成一个假设性的答案文档 。然后，使用  的嵌入  来检索真实的文档块。这种方法认为，与查询直接相关的文档，其语义可能更接近于一个理想的答案，而不是查询本身。
...
+ **检索中策略 (During-retrieval Strategies)**: 这些策略在检索过程中动态调整。
    - **嵌入模型微调 (Fine-tuning Embedding Models)**: 在特定领域的数据集或与任务相关的查询-文档对上微调嵌入模型。可以使用对比学习损失（如 InfoNCE）来拉近相关对的距离，推远不相关对的距离。其中  是与查询  相关的正例文本块嵌入， 是负例文本块嵌入（通常包括批内负例和难负例）， 是温度超参数。BGE M3-Embedding ([Chen et al., 2024](https://arxiv.org/abs/2402.03216)) 通过自知识蒸馏和高效批处理策略训练多语言、多功能、多粒度的文本嵌入。
...
+ **后检索策略 (Post-retrieval Strategies)**: 这些策略在检索到初步结果后进行优化。
    - **重排序 (Re-ranking)**: 对初步检索到的  个文本块进行二次排序，以提高排序顶部的相关性。
...
+  是用户查询，包含  个查询词。
+  是一个待评分的文档。
+  是查询词  的逆文档频率，衡量词语的稀有程度。计算方式通常为：其中  是文档总数， 是包含查询词  的文档数。
+  是查询词  在文档  中的词频。
+  是文档  的长度（通常指词数）。
+  是数据集中所有文档的平均长度。
+  和  是可调参数：
    -  (通常取值在 1.2 到 2.0 之间) 控制词频饱和度，即词频对相关性得分的非线性影响。较高的  值意味着词频对得分的贡献更大，直到达到一个饱和点。
    -  (通常取值在 0.75 左右) 控制文档长度归一化的程度。当  时，文档长度的影响被完全归一化；当  时，不考虑文档长度。
...
+  是一个待评分的文档。
+  是参与融合的检索系统（例如，一个 BM25 检索器和一个向量检索器）的集合。
+  是文档  在检索系统  返回结果列表中的排名（从1开始）。如果文档  未被系统  检索到，则其排名可以视为无穷大，或者该项不参与求和。
+  是一个小常数（例如 60），用于平滑排名影响，减少排名靠前但仅被一个系统检索到的文档的权重过高问题。
...
**Fig. 8. The GraphRAG pipeline using LLM-derived graph indexing. (Image source: **[**Edge et al., 2024**](https://syhya.github.io/zh/posts/2025-02-03-rag/#edge-et-al-2024)**)**
...
## Agentic RAG
**Agentic RAG** ([Singh et al., 2025](https://syhya.github.io/zh/posts/2025-02-03-rag/#singh-et-al-2025)) 是一种更高级的 RAG 范式，它将自主代理（AI Agents）集成到 RAG 流程中。这些代理能够进行动态决策、规划、使用工具并进行协作，从而实现更灵活、更智能的检索和生成过程。

![](/images/yuque/040-rag-2/image-5-b40936f9.jpeg)

**Fig. 9. Overview of AI Agent components. (Image source: **[**Singh et al., 2025**](https://syhya.github.io/zh/posts/2025-02-03-rag/#singh-et-al-2025)**)**
...
随着信息的多样化，仅仅处理文本数据已不能满足许多应用场景的需求。**多模态 RAG (Multimodal RAG)** ([Abootorabi et al., 2025](https://syhya.github.io/zh/posts/2025-02-03-rag/#abootorabi-et-al-2025); [Zhao et al., 2023a](https://syhya.github.io/zh/posts/2025-02-03-rag/#zhao-et-al-2023a)) 将 RAG 的能力扩展到多种数据模态，如文本、图像、音频、视频等。它旨在通过从包含多种类型数据的外部知识库中检索和融合信息，来增强多模态大型语言模型 (MLLMs) 的生成能力。

![](/images/yuque/040-rag-2/image-6-8b44aac5.jpeg)

**Fig. 10. Overview of the Multimodal Retrieval-Augmented Generation (RAG) pipeline. (Image source: **[**Abootorabi et al., 2025**](https://syhya.github.io/zh/posts/2025-02-03-rag/#abootorabi-et-al-2025)**)**

### 任务定义
给定一个多模态查询  和一个包含多种模态文档  的语料库，多模态 RAG 的目标是生成一个相关的多模态响应 。 每个文档  具有其自身的模态 。首先，使用特定于模态的编码器  将每个文档  编码为一个向量表示 ：

这些编码器通常旨在将不同模态的信息映射到一个共享的语义空间，以便进行跨模态对齐和比较。 然后，检索模型  根据查询 （其编码表示为 ）与每个编码后的文档  之间的相关性评分  来选择最相关的文档子集 ：

其中  是针对模态  的相关性阈值。 最后，生成模型  利用原始查询  和检索到的多模态上下文  来生成最终的多模态响应 ：
...
5. **多模态生成技术 (Multimodal Generation Techniques)**
    - **上下文学习 (In‑Context Learning, ICL)：** 利用检索到的多模态内容作为少样本示例，无需重新训练即可增强 MLLM (如 GPT‑4V ([OpenAI, 2023](https://arxiv.org/abs/2303.08774)), LLaVA ([Liu et al., 2023a](https://arxiv.org/abs/2304.08485)), Gemini (_暂无 arXiv 预印本_)) 的推理能力。Raven ([Rao et al., 2024](https://arxiv.org/abs/2308.07922)) 提出 Fusion‑in‑Context Learning。
...
6. **训练策略与损失函数 (Training Strategies and Loss Functions):**
    - **对齐 (Alignment):** 主要使用对比学习（Contrastive Learning）来提高表示质量，常用损失函数为 InfoNCE loss：其中  是正样本对的嵌入， 是负样本嵌入， 是温度参数。
    - **生成 (Generation):**
        * 文本生成：通常使用交叉熵损失 (Cross-Entropy Loss)。
        * 图像生成：常用生成对抗网络 (GANs) 或扩散模型 (Diffusion Models)。GAN 的损失函数包括二元交叉熵、Wasserstein 损失等。扩散模型通常使用均方误差 (MSE) 损失进行噪声预测。
...
+ **忠实度 (Faithfulness):**
    - **定义:** 衡量生成的答案  是否完全基于给定的上下文  得出，即答案中的声明是否都能从上下文中推断出来。这个指标对于避免幻觉至关重要。
    - **计算方法 (Ragas):**
        1. 使用 LLM 从生成的答案  中提取一组声明（statements）。
        2. 对于每个声明 ，再次使用 LLM 验证该声明是否可以从上下文  中推断出来。得到一个验证为真的声明集合 。
        3. 忠实度得分  计算为：其中  是被验证为真实的声明数量， 是从答案中提取的声明总数。得分范围为 ，越高越好。
+ **答案相关性 (Answer Relevance):**
    - **定义:** 衡量生成的答案  与原始问题  的相关程度。它惩罚不完整或包含冗余信息的答案。
    - **计算方法 (Ragas):**
        1. 给定生成的答案 ，使用 LLM 生成  个可能的反向问题 ，这些问题应该可以由答案  来回答。
        2. 计算每个生成的反向问题  与原始问题  之间的嵌入相似度 （例如使用 OpenAI text-embedding-ada-002 计算余弦相似度）。
        3. 答案相关性得分  计算为这些相似度的平均值：得分范围为 ，越高越好。
+ **上下文精确率 (Context Precision) / 上下文相关性 (Context Relevance):**
    - **定义:** 衡量检索到的上下文  中包含了多少回答问题  所必需的信息，同时惩罚包含过多无关或冗余信息的情况。Ragas 中更侧重于上下文的“精确性”，即上下文中与问题相关的句子比例。
    - **计算方法 (Ragas 中的 Context Precision):**
        1. 给定问题  和检索到的上下文 ，使用 LLM 从上下文  中提取出对回答问题  至关重要的句子集合 。
        2. 上下文精度 (Context Precision) 得分  计算为：其中  是从上下文  中提取的关键句数量， 是上下文  中的总句数。得分范围为 ，越高越好。
+ **上下文召回率 (Context Recall):** (需要真实答案 Ground Truth)
    - **定义:** 衡量检索到的上下文  是否包含了回答问题  所需的所有真实答案  中的信息。
    - **计算方法 (Ragas):** 对于真实答案  中的每个句子，判断其是否可以从检索到的上下文  中归因得到。上下文召回率  是可归因的真实答案句子数  与真实答案总句数  的比率。
...
+ **平均倒数排名 (Mean Reciprocal Rank, MRR)**: 评估检索结果中第一个相关文档的排名。对于一组查询 ，MRR 计算如下：其中  是第  个查询的第一个相关文档的排名。
+ **归一化折扣累积增益 (Normalized Discounted Cumulative Gain, NDCG@k)**: 考虑检索结果的排序和相关性等级。其中  是排名第  位的文档的相关性得分， 是理想排序下的 DCG@k 值。

### 3. 其他生成质量评估指标
+ **BLEU (Bilingual Evaluation Understudy)** ([Papineni et al., 2002](https://aclanthology.org/P02-1040/)): 基于 N-gram 精确率。
+ **ROUGE (Recall-Oriented Understudy for Gisting Evaluation)** (Lin, 2004): 基于 N-gram 召回率。
